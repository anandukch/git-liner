...
class GitClient {
  constructor() {
    ...
    !fs.existsSync(`${os.homedir()}/.ghcli`) &&
      fs.mkdirSync(`${os.homedir()}/.ghcli`);
  ...
  createGitClient = (GITHUB_TOKEN) => {
    if (!GITHUB_TOKEN) {
      GITHUB_TOKEN = this.readCredentials();
    }
  ...
  pushCommand(projName, username, existing = false) {
    return `${
      !existing ? "git init &&" : ""
    } git add . && git commit -m "publishing" && git branch -M master && git remote add origin https://github.com/${username}/${projName}.git && git push -u origin master`;
  }

  async getUserName() {
    return await this.git.request("GET /user", {
      headers: {
        "X-GitHub-Api-Version": "2022-11-28",
      },
    });
